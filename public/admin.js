...
async function fetchLogs() {
    ...
    console.log(
        `fetchLogs: Requesting Page=${currentPage}, Limit=${limit}, Sort=${sortBy}, Order=${sortOrder}`,
    );
    clearError();
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(
                `Failed to fetch logs: ${response.status} ${response.statusText} - ${errorData}`,
            );
        }
        ...
        if (!data.success) {
            throw new Error(
                data.message || "API returned unsuccessful status.",
            );
        }
...
function renderPagination(pageToRenderActive, totalPagesToRender) {
    console.log(
        `renderPagination: Rendering for page ${pageToRenderActive} of ${totalPagesToRender}`,
    ); // Debug render call
    ...
    const createButton = (
        ...
        if (!isDisabled && !isActive) {
            button.addEventListener("click", () => {
                ...
                console.log(
                    `Pagination Click: Button Value=${pageNumValue}, Current Page Before=${currentPage}`,
                );
                ...
                console.log(
                    `Pagination Click: Current Page After=${currentPage}`,
                );
    ...
    let startPage = Math.max(
        1,
        pageToRenderActive - Math.floor(maxPagesToShow / 2),
    );
...
tableHeaders.forEach((th) => {
    ...
    if (sortKey) {
        ...
        th.addEventListener("click", () => {
            console.log(
                `Sort Clicked: Column=${sortKey}, Current Sort=${sortBy}, Current Order=${sortOrder}`,
            );
            if (sortBy === sortKey) {
            ...
            console.log(
                `Sort Clicked: New Sort=${sortBy}, New Order=${sortOrder}`,
            );
            currentPage = 1; // Reset to first page
            fetchLogs();
        });
