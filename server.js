...
const {
    ...
    photoUploaded,
    getPhotoCount,
} = require("./log-service"); // Add this line

// Configuration
const SESSION_TIMEOUT_MS = 600000; // 600 seconds
const CLIENT_TIMEOUT_MS = 3000; // 3 seconds
const UPLOAD_DIR = path.join(__dirname, ".data", "uploads");
const ADMIN_CREDENTIALS = "admin:1234"; // e.g., "admin:password123"
const POLLING_INTERVAL_MS = 1000; // How often clients should poll (informational for client)
...
// remove? const glitchImageUrl = 'https://cdn.glitch.global/7a08d29a-3fe4-44c5-8a06-ff5e944d8c35/QuickBeam_Logo.png?v=1744798242240'
...
async function cleanupSession(sessionId, reason) {
    ...
    if (imagePathsToDelete.length > 0) {
        fastify.log.info({
            msg: `Cleanup attempting to delete ${imagePathsToDelete.length} image file(s)`,
            sessionId,
        ...
        for (const imagePath of imagePathsToDelete) {
            try {
                ...
                fastify.log.info({
                    msg: `Cleaned up image file via cleanupSession`,
                    sessionId,
                    imagePath,
                });
            } catch (err) {
                if (err.code !== "ENOENT") {
                    fastify.log.error({
                        msg: `Error cleaning up image file during cleanupSession`,
                        sessionId,
                        imagePath,
                    ...
                    fastify.log.warn({
                        msg: `Image file already deleted when cleanupSession ran`,
                        sessionId,
                        imagePath,
                    });
                }
...
function runExpiredSessionCleanup() {
    ...
    if (cleanedCount > 0) {
        fastify.log.info(
            `Background cleanup task removed ${cleanedCount} expired sessions.`,
        );
    }
}
...
async function checkAdminAuth(req, reply) {
    ...
    try {
        ...
        fastify.log.info({
            msg: "Admin authentication successful",
        ...
        fastify.log.error({
            msg: "Error decoding admin auth token",
            error: e.message,
        });
        reply.code(400).send({
            success: false,
...
fastify.post("/api/session/request", async (req, reply) => {
    ...
    fastify.log.info({
        msg: "Processing requestPairing REST",
    ...
    try {
        ...
        fastify.log.info({
            msg: `Session created via REST, QR sent to receiver`,
            sessionId,
        });
    } catch (qrError) {
        fastify.log.error({
        ...
        reply
            .code(500)
            .send({ success: false, message: "Failed to generate QR code." });
    }
});
...
fastify.post("/api/session/:sessionId/connect", async (req, reply) => {
    ...
    const session = sessions[sessionId];
    fastify.log.info({
        msg: "Sender connect REST attempt",
        sessionId,
        ip: req.ip,
    });
    ...
    if (!session) {
        fastify.log.warn({
            msg: "Sender connect failed: Invalid session ID",
    ...
    if (session.status !== "waiting_for_sender") {
        ...
        if (session.status === "connected" && session.senderLastSeen > 0) {
            fastify.log.warn({
                msg: "Sender connect failed: Session already has an active sender",
                sessionId,
                ip: req.ip,
            });
            return reply.code(409).send({
                success: false,
                message: "Session already has an active sender.",
            });
        }
    ...
    fastify.log.info({
        msg: `Sender connected successfully via REST`,
        sessionId,
        ip: req.ip,
    });
    reply.code(200).send({
...
fastify.post("/api/session/:sessionId/status", async (req, reply) => {
    ...
    if (!clientType || (clientType !== "receiver" && clientType !== "sender")) {
        return reply.code(400).send({
            success: false,
            message: "Missing or invalid client type query parameter.",
        });
    }
    if (!session) {
        ...
        return reply
            .code(404)
            .send({ success: false, message: "Session not found or expired." });
    }
    ...
    if (clientType === "receiver") {
        ...
        if (
            session.status === "connected" &&
            now > session.senderLastSeen + CLIENT_TIMEOUT_MS
        ) {
            session.status = "sender_disconnected";
            otherClientDisconnected = true;
            fastify.log.warn({
                msg: "Sender timed out based on receiver poll",
                sessionId,
            });
        }
    } else {
        ...
        if (
            session.status === "connected" &&
            now > session.receiverLastSeen + CLIENT_TIMEOUT_MS
        ) {
            session.status = "receiver_disconnected";
            otherClientDisconnected = true;
            fastify.log.warn({
                msg: "Receiver timed out based on sender poll",
                sessionId,
            });
        }
    }
    ...
    if (clientType === "receiver" && session.pendingImageIds.length > 0) {
        ...
        fastify.log.info({
            msg: "Delivering pending image IDs to receiver poll",
            sessionId,
            count: responsePayload.newImageIds.length,
            imageIds: responsePayload.newImageIds,
        });
    }
...
fastify.get("/sender", (req, reply) => {
    ...
    if (!sessionId || !sessions[sessionId]) {
        fastify.log.warn({
            msg: "Sender page access attempt with invalid/expired SessionId",
            sessionId,
            ip: req.ip,
        });
...
fastify.post("/upload", async (req, reply) => {
    ...
    fastify.log.info({
        msg: "Upload POST request received",
        sessionId,
        ip: req.ip,
    });
    ...
    if (!sessionId || !session) {
        fastify.log.warn({
            msg: "Upload rejected: Invalid session ID",
            sessionId,
            ip: req.ip,
        });
        return reply.code(400).send({
            success: false,
            message: "Missing, invalid or expired session ID.",
        });
    }
    ...
    if (session.status !== "connected") {
        fastify.log.warn({
            msg: "Upload rejected: Session not in connected state",
            sessionId,
        ...
        if (session.status === "waiting_for_sender")
            userMessage = "Sender has not connected yet.";
    ...
    if (now > session.receiverLastSeen + SESSION_TIMEOUT_MS) {
        ...
        fastify.log.warn({
            msg: "Upload rejected: Receiver timed out just before upload",
            sessionId,
            ip: req.ip,
        });
        cleanupSession(
            sessionId,
            "Receiver timed out before upload could complete",
        ); // Clean up session
        return reply.code(400).send({
            success: false,
            message: "Receiver partner disconnected.",
        });
    }
    ...
    fastify.log.info({
        msg: "Reset session inactivity timeout on upload",
        sessionId,
    });
    ...
    try {
        ...
        if (!data) {
            fastify.log.warn({
                msg: "Upload rejected: No file data in request",
                sessionId,
                ip: req.ip,
            });
            return reply
        ...
        fastify.log.info({
            msg: "Generated image path for upload",
            sessionId,
            imageId,
            imagePath,
            ip: req.ip,
        ...
        fastify.log.info({
            msg: "Image file successfully written to disk",
            sessionId,
            imagePath,
            ip: req.ip,
        });
        ...
        fastify.log.info({
            msg: "Added new image to session maps (paths and pending)",
            sessionId,
            imageId,
            pendingCount: session.pendingImageIds.length,
        });

        // --- Log Upload Event ---
        logEvent(req.ip, sessionId, Actions.UPLOAD_IMAGE, {
            imageId: imageId,
            fileSize: fileSize,
        });
        // --- End Log ---
        ...
        fastify.log.error({
            msg: "Image upload processing error",
        ...
        if (
            ...
            (error.code === "ERR_STREAM_PREMATURE_CLOSE" &&
                req.isMultipartLimitError) ||
            error.message.toLowerCase().includes("limit")
        ) {
            return reply.code(413).send({
                success: false,
                message: `File upload error: Limit exceeded. ${error.message}`,
            });
        }
...
fastify.get("/image/:sessionId/:imageId", async (req, reply) => {
    ...
    fastify.log.info({
        msg: "Image GET request received",
        sessionId,
        imageId,
        ip: req.ip,
    });
    ...
    if (!sessionId || !session) {
        fastify.log.warn({
            msg: "Image GET failed: Session not found",
            sessionId,
            imageId,
            ip: req.ip,
        });
        return reply.code(404).send({
            success: false,
            message: "Image not found or session invalid.",
        });
    }

    const imagePath = session.imagePaths[imageId];
    fastify.log.info({
        msg: "Image GET: Session found, checking imagePath for imageId",
        sessionId,
        imageId,
        currentImagePathInSession: imagePath,
        ip: req.ip,
    });

    if (!imagePath) {
        fastify.log.warn({
            msg: "Image GET failed: imageId not found in session path map",
            sessionId,
            imageId,
            ip: req.ip,
        });
        return reply.code(404).send({
            success: false,
            message: "Image not found (invalid image ID for this session).",
        });
    }
    ...
    try {
        ...
        logEvent(req.ip, sessionId, Actions.DOWNLOAD_IMAGE, {
            imageId: imageId,
            fileSize: fileSize,
        });
        // --- End Log ---
        ...
        fastify.log.info({
            msg: "Image GET: Buffered response sending initiated",
            sessionId,
            imageId,
            mimeType,
        ...
        process.nextTick(async () => {
            try {
                fastify.log.info({
                    msg: "Image GET: Attempting post-send cleanup",
                    sessionId,
                    imageId,
                    imagePath,
                });
                await fs.unlink(imagePath);
                fastify.log.info({
                    msg: `Successfully deleted image after sending`,
                    sessionId,
                    imageId,
                    imagePath,
                });
                ...
                if (currentSession && currentSession.imagePaths[imageId]) {
                    ...
                    fastify.log.info({
                        msg: `Removed image path from session map`,
                        sessionId,
                        imageId,
                        remainingImages: Object.keys(currentSession.imagePaths)
                            .length,
                    });
                } else {
                    fastify.log.warn({
                        msg: `Session or imageId entry gone before post-send map cleanup`,
                        sessionId,
                        imageId,
                    });
                }
                ...
                if (unlinkError.code !== "ENOENT") {
                    fastify.log.error({
                        msg: `Error deleting image after sending`,
                        sessionId,
                        imageId,
                        imagePath,
                    ...
                    fastify.log.warn({
                        msg: `Image already deleted during post-send cleanup`,
                        sessionId,
                        imageId,
                        imagePath,
                    });
        ...
        if (error.code === "ENOENT") {
            fastify.log.warn(
                `Image GET failed: File not found (ENOENT on access/read)`,
                {
                    sessionId,
                    imageId,
                    imagePath,
            ...
            reply
                .code(404)
                .send({ success: false, message: "Image file not found." });
        } else if (
            error instanceof Error &&
            error.message.includes("cannot be larger than")
        ) {
            fastify.log.error({
                msg: "Image GET: Error reading file into buffer (likely too large)",
                sessionId,
                imageId,
                imagePath,
                error: error.message,
            ...
            reply.code(500).send({
                success: false,
                message: "Failed to read image file (too large).",
            });
        } else {
            fastify.log.error({
                msg: "Image GET: Error during file access/read",
                sessionId,
                imageId,
                imagePath,
                error: error.message,
...
fastify.get("/admin", { preHandler: [checkAdminAuth] }, async (req, reply) => {
    ...
    fastify.log.info(
        "Admin route handler starting, attempting to render view...",
    ); // Add log
    try {
        ...
        fastify.log.info(
            "Admin view rendering completed successfully via await.",
        ); // Add log
    } catch (viewError) {
        // Log the error if rendering fails
        fastify.log.error({
            msg: "Error occurred during reply.view rendering",
            error: viewError,
        });
...
fastify.get("/admin/logout", async (req, reply) => {
    ...
    reply
        ...
        .send(
            "<html><body><h1>Logged Out</h1><p>You have been logged out. Please close this browser window/tab for security.</p></body></html>",
        );
            ...
            const allLogs = lines
                .map((line) => {
                    try {
                        ...
                        fastify.log.warn({
                            msg: "Failed to parse log line, skipping",
                            line: line,
                            error: e.message,
                        });
